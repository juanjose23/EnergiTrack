    ...
    <div>
        <div class="row">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <h4 class="py-3 mb-4"><span class="text-muted fw-light">Gestión Planes /</span> Planes</h4>
                                ...
                                @can('create', App\Models\Categorias::class)
                                    <div class="btn-group me-2 mb-2 mb-md-0">
                                        <a href="{{ route('plan.create') }}" class="btn btn-primary">
                                            <i class='bx bx-plus'></i> Registrar plan
                                        </a>
                                    </div>
                                @endcan
                        ...
                        <table class="table">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">
                                        <a wire:click="ordenar('dispositivos')">
                                            Dispositivos
                            ...
                            <tbody>
                                @foreach ($planes as $plan)
                                    <tr>
                                        ...
                                        <td class="text-wrap">{{ wordwrap($plan->descripcion, 50, "\n", true) }}</td>
                                        <td><span
                                                class="badge  {{ $plan->estado == 1 ? 'bg-label-primary ' : 'bg-label-danger' }}">
                                            ...
                                            <div class="d-inline-block text-nowrap">
                                                ...
                                                @can('delete', App\Models\Categorias::class)
                                                    <button type="button" class="btn btn-sm btn-icon delete-record"
                                                        role="button" onclick="confirmAction({{ $plan->id }})">
                                                        <i
                                                            class="bx bx-{{ $plan->estado == 1 ? 'trash' : 'toggle-right' }}"></i>
                                                    </button>
                                                @endcan
                                            </div>

                                            <form id="deleteForm{{ $plan->id }}"
                                                action="{{ route('plan.destroy', ['plan' => $plan->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')

                                                <!-- Este botón no es visible, pero se utilizará para activar el SweetAlert -->
                                                <button id="submitBtn{{ $plan->id }}" type="submit"
